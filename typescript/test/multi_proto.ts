...
const GENERATED_PROTO_FILE =
    path.join(OUTPUT_DIR, 'src', 'v1', 'keymanagementservice_proto_list.json');
const GOOGLE_GAX_PROTOS_DIR =
    path.join(cwd, 'node_modules', 'google-gax', 'protos');
const PROTOS_DIR = path.join(cwd, 'build', 'test', 'protos');
const KMS_PROTO_FILE =
    path.join(PROTOS_DIR, 'google', 'kms', 'v1', 'service.proto');

const PROTOLIST_LIBRARY_BASELINE = path.join(
    cwd, 'typescript', 'test', 'testdata',
...
describe('Proto List Generate Test', () => {
  describe('Generate Client library', () => {
    it('Generated proto list should have same output with baseline.',
       function() {
         this.timeout(10000);
